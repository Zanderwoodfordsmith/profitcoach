-- Support nav badge = unread tickets that need a look, not only follow-up chats.
-- New assigned tickets (original body, never opened) count as 1.
-- Follow-up replies from someone else still count on that same ticket.
...
begin
  ...
  from (
    select * from unread_follow_ups
    union all
    select * from unread_new
    union all

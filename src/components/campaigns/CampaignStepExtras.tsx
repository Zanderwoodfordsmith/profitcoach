...
export function MessageStepMedia({
  ...
  const selected: CampaignStepMediaKind | "message" = kind ?? "message";
  const signedUrl =
    media && typeof (media as CampaignStepMedia & { signedUrl?: string }).signedUrl === "string"
      ? (media as CampaignStepMedia & { signedUrl?: string }).signedUrl
      : media &&
          typeof (config as { media?: { signedUrl?: string; signed_url?: string } } | null)
            ?.media?.signedUrl === "string"
        ? (config as { media?: { signedUrl?: string } }).media?.signedUrl
        : typeof (config as { media?: { signed_url?: string } } | null)?.media
            ?.signed_url === "string"
          ? (config as { media?: { signed_url?: string } }).media?.signed_url
          : null;

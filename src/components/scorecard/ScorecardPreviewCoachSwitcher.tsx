...
export function ScorecardPreviewCoachSwitcher({
  currentSlug,
  alwaysShow = false,
  floating = false,
}: {
  ...
  const [canSwitch, setCanSwitch] = useState(alwaysShow);
  ...
  useEffect(() => {
    ...
    fetch(`/api/coach-by-slug?slug=${encodeURIComponent(currentSlug)}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (data?.full_name?.trim()) {
          setDisplayName(data.full_name.trim());
        } else if (data?.coach_business_name?.trim()) {
          setDisplayName(data.coach_business_name.trim());
        }
      })
      .catch(() => {});
  }, [currentSlug]);
  ...
  const currentCoach = useMemo(
    () => coaches.find((c) => c.slug === currentSlug),
    [coaches, currentSlug]
  );
  ...
  const loadCoaches = useCallback(async () => {
    ...
    try {
      ...
      if (!session?.access_token) {
        setLoadError("Sign in as admin or coach to switch.");
        setCoaches([]);
        return;
      }
  ...
  function selectCoach(slug: string) {
    if (slug === currentSlug) {
      setOpen(false);
      return;
    }
  ...
  if (!canSwitch && !displayName) return null;

  const label =
    currentCoach
      ? coachDisplayName(currentCoach)
      : displayName ?? currentSlug ?? "Coach";

  return (
    <div ref={rootRef} className={`relative min-w-0 ${floating ? "w-full" : "ml-auto"}`}>
      <button
        type="button"
        onClick={handleToggleOpen}
        className={`group flex min-w-0 items-center gap-1 rounded-lg border px-3 py-1.5 text-left ${
          floating
            ? "w-full border-amber-200/80 bg-amber-50/50 shadow-sm"
            : "max-w-[12rem] border-slate-200 bg-white shadow-sm sm:max-w-[16rem]"
        }`}
        title="Preview as another coach"
        aria-expanded={open}
      ...
      {open ? (
        <div
          className={`absolute right-0 z-[110] w-[min(18rem,calc(100vw-5rem))] rounded-lg border border-slate-200 bg-white py-2 shadow-lg ring-1 ring-black/5 ${
            floating
              ? "bottom-[calc(100%+6px)]"
              : "top-[calc(100%+6px)]"
          }`}
          role="listbox"
          aria-label="Preview coaches"
        >

...
export async function GET(request: Request) {
  ...
  if (error) {
    console.error("coach-chat-folders GET error:", error);
...
export async function POST(request: Request) {
  ...
  if (parentId) {
    ...
    if (parent.parent_id !== null) {
      return NextResponse.json({ error: "Only one level of subfolder is allowed." }, { status: 400 });
    }
  }
  ...
  if (insertError) {
    console.error("coach-chat-folders POST error:", insertError);

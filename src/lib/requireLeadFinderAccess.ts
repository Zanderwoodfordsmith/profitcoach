...
export async function requireLeadFinderAccess(request: Request): Promise<
  | {
      error:
...
> {
  try {
    ...
    if (
      ...
      !profile ||
      profile.role !== "admin" ||
      !isLeadFinderAllowedEmail(user.email)
    ) {
    ...
    console.error("requireLeadFinderAccess error:", err);
